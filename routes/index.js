...
/*  const { productname, customername, sunnyorderid, status } = req.body */
  ...
  pool.query('INSERT INTO logistics (id,productname, customername, sunnyorderid, status) VALUES ((select coalesce(max(id),0)+1 from logistics),$1, $2, $3, $4)', [productname, customername, sunnyorderid, status], (error, results) => {
    if (error) {
      throw error
    }
